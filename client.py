...
class routePlanner():
    ...
    def readinData(self, patients_readin, hospitals_readin, ambulances):
        ...
        self.__dist_mat = sp.spatial.distance_matrix(np.vstack((self.__patients[:, :2], hospital_array[::-1])),
                                                     np.vstack((self.__patients[:, :2], hospital_array[::-1])), 1)
    ...
    def chromsomeInit(self, c_num):
        ...
        for e in cluster[amb_num:]:
            ...
            for ind, route in enumerate(routes):
                savings = self.__dist_mat[last_h[ind], route[-1]] + self.__dist_mat[last_h[ind], e] - self.__dist_mat[
                    route[-1], e]
            ...
            if (len(routes[max_ind]) > 3) & (not np.any(routes[max_ind][-3:]) < 0):
                add_h = True
            else:
    ...
    def crossoverInsert(self, route, parent):
        for el in [e for e in route if e > 0]:
            for d in self.__dist_mat[parent, el].argsort():
                if parent[d] < 0:
                    continue
                if d < 3:
                    if (min(parent[-3 + d:]) < 0) | (min(parent[d:d + 3]) < 0):
                        parent.insert(d, el)
                        break
                else:
                    if min(parent[d - 3:d + 3]) < 0:
                        parent.insert(d, el)
                        break

        # print('new child', parent)
        return parent
    ...
    def crossover(self, chrm1, chrm2):
        ...
        r1 = p1[amb1[i1 - 1]:amb1[i1]]
        r2 = p2[amb2[i2 - 1]:amb2[i2]]
        ...
        pr1 = [e for e in p1 if (e < 0) | (e not in r2)]
        pr2 = [e for e in p2 if (e < 0) | (e not in r1)]
        ...
        ch1 = self.crossoverInsert(r1, p2)
        ch2 = self.crossoverInsert(r2, p1)

        return [ch1, amb2], [ch2, amb1]

    def mutate(self):
        for i in range(self.__mutate_size):
            c = np.random.choice(np.arange(0, self.__pop_size))
            c1, a1 = self.__pop[c]
            r1_ind = np.random.randint(0, len(a1))
            r1_ind = 1 if r1_ind == 0 else r1_ind
            r1 = c1[a1[r1_ind - 1]:a1[r1_ind - 1]]
            r1 = r1[::-1]
            c1[a1[r1_ind - 1]:a1[r1_ind] - 1] = r1
            self.__pop[c][0] = c1
