...
class routePlanner():
    def __init__(self, patients, hospitals, ambulances, pop_size, num_iter):
        ...
        self.__crossover_size = int(pop_size * 0.3)
        self.__save_size = int(pop_size * 0.3)
        self.__new_size = int(pop_size * 0.4)
        self.__mutate_size = int(pop_size * 0.05)
    ...
    def readinData(self, patients_readin, hospitals_readin, ambulances):
        ...
        self.__hospital_array = [self.__hospital_loc[i[0]] for i in hospital_assignment]

        self.__ambulances = ambulances
        self.__dist_mat = sp.spatial.distance_matrix(np.vstack((self.__patients[:,:2], self.__hospital_array[::-1])), np.vstack((self.__patients[:,:2], self.__hospital_array[::-1])), 1)    
    ...
    def evolve(self):
        ...
        for i in range(self.__num_iter):
            ...
            if pop_cost.max() > self.__best_cost:
                self.__best_chromosome = self.__pop[pop_cost.argmax()]
                self.__best_cost = pop_cost.max()
            new_pop = self.selection(pop_cost, self.__crossover_size)  
            new_pop += [self.__pop[i] for i in pop_cost.argsort()[::-1][:self.__save_size]]
            new_pop += self.extraInit(self.__new_size)
            self.__pop = new_pop
            self.mutate()
    ...
    def extraInit(self, k):       
        
        new_pop = [[] for i in range(k)]
        ...
        for c in range(k):
            ...
            new_pop[c] = [list(chain(*route)), np.cumsum(list(chain(*amb)))]
        return new_pop
    ...
    def crossover(self, chrm1, chrm2):
        ...
        r1 = p1[amb1[i1-1]-1:amb1[i1]].copy()
        r2 = p2[amb2[i2-1]-1:amb2[i2]].copy()
    ...
    def writeDictionaries(self):
        ...
        for amb_num in range(len(amb)):
            ...
            ambulance_routes[amb_num] = route
        return hospitals, ambulance_routes
